...
class Command(BaseCommand):
    def add_arguments(self, parser):
        parser.add_argument("--livros", default="population/livros.csv")
        parser.add_argument("--editoras", default="population/editora.csv")
        parser.add_argument("--autores", default="population/autores.csv")
        parser.add_argument("--truncate", action="store_true")
        parser.add_argument("--update", action="store_true")

    @transaction.atomic
    def handle(self, *a, **o):
        df_autores = pd.read_csv(o["autores"], encoding="utf-8-sig")
        df_editoras = pd.read_csv(o["editoras"], encoding="utf-8-sig")
        df_livros = pd.read_csv(o["livros"], encoding="utf-8-sig")
        ...
        if o["truncate"]:Livro.objects.all().delete() # apaga tudo
        
        df_autores['nome_completo'] = df_autores['name'].str.strip()+" "+df_autores['last_name'].str.strip() 
        # strip tira o espaço no começo e no fim, não tira no meio
        ...
        df_livros['autor'] = df_livros['autor'].astype(int)
        df_livros['editora'] = df_livros['editora'].astype(int)
        ...
        df_livros['preco'] = df_livros['preco'].astype(int)
        ...
        df_livros.to_excel('livro.xlsx', index=False) # cria o arquivo 
